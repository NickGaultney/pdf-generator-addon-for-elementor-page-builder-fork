...
class Mpdf implements \Psr\Log\LoggerAwareInterface
{
	...
	function SetHTMLHeader($header = '', $OE = '', $write = false)
	{
		...
		if ($this->setAutoTopMargin == 'pad') {
			...
			$rtwwpf_css_stng = get_option('rtwwpf_css_setting_opt');
			if( isset( $rtwwpf_css_stng['body_top_margin'] ) && !empty( $rtwwpf_css_stng['body_top_margin'] ) ){
				$sgb_top_marg = $rtwwpf_css_stng['body_top_margin'];	
			}else {
				$sgb_top_marg = 16;
			}
			$this->tMargin = max($this->orig_tMargin, $this->margin_header + $height + $this->autoMarginPadding, $sgb_top_marg);
			if (isset($this->saveHTMLHeader[$this->page][$OE]['mt'])) {
				$this->saveHTMLHeader[$this->page][$OE]['mt'] = $this->tMargin;
			}
		}
	...
	function Footer()
	{
		...
		if (($this->watermarkText) && ($this->showWatermarkText)) {
			$rtwwcfp_get_setting = get_option('rtwwpf_watermark_setting_opt');
			if(isset($rtwwcfp_get_setting['watermark_rotation']) && !empty($rtwwcfp_get_setting['watermark_rotation'])){
		  		$rotate_custom = $rtwwcfp_get_setting['watermark_rotation'];
		  	}else{
		  		$rotate_custom = 45;
		  	}
			$this->watermark($this->watermarkText, $rotate_custom, 120, $this->watermarkTextAlpha); // Watermark text
		}
